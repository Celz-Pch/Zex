...
// Color pair IDs (ZP_ prefix to avoid ncurses PAIR_NUMBER clash)
#define ZP_NORMAL    1
#define ZP_KEYWORD   2
#define ZP_STRING    3
#define ZP_COMMENT   4
...
typedef enum { FOCUS_TREE, FOCUS_EDITOR } focus_t;
...
// --- editor initialization (editor_init.c) ---
...
// --- draw status (draw_status.c) ---
void        draw_status_bar(editor_t *ed);

// --- syntax highlighting (syntax.c) ---
void        emit(WINDOW *win, const char *s, int len, int pair, int attr);
int         is_keyword(const char *word, int len);
void        highlight_line(WINDOW *win, int row, const char *line,
                int max_cols);

// --- token handlers (hl_tokens.c) ---
...
// --- editor key dispatch (editor_keys.c) ---
